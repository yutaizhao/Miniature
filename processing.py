...
class ImageToolbox:
    @staticmethod
    def _circular_kernel(size: int, mode: str) -> np.ndarray:
        # Ensure odd size
        if size % 2 == 0:
            size += 1
        r = size // 2
        ...
        if mode == "circular":  # uniform disk
            kernel[dist <= r] = 1.0
        elif mode == "circular_soft":  # smooth falloff
            mask = dist <= r
            kernel[mask] = 1.0 - (dist[mask] / r)
        elif mode == "circular_sharp":  # strong edge emphasis
            mask = dist <= r
            t = dist[mask] / r
            kernel[mask] = np.clip((1.0 - t) ** 3, 0, 1)
        s = kernel.sum()
        if s > 0:
            kernel /= s
        return kernel
    ...
    @staticmethod
    def _polygon_kernel(size: int, n_blades: int) -> np.ndarray:
        if size % 2 == 0:
            size += 1
    ...
    def apply_enhancements(
        image: np.ndarray, saturation: float, contrast: float
    ) -> np.ndarray:
    ...
    def apply_blur_style(image: np.ndarray, level: int, shape: str) -> np.ndarray:
        ...
        elif shape in ("circular", "circular_soft", "circular_sharp"):
            kernel = ImageToolbox._circular_kernel(k_size, shape)
            return cv2.filter2D(image, -1, kernel)
        elif shape in ("6_blades", "8_blades", "9_blades"):
            n = int(shape.split("_")[0])
            kernel = ImageToolbox._polygon_kernel(k_size, n)
            return cv2.filter2D(image, -1, kernel)
        return cv2.GaussianBlur(image, (k_size, k_size), 0)
...
class TiltShiftZoneDefiner:
    ...
    def __init__(self, shape: Tuple[int, ...], settings: MiniatureSettings):
        ...
        self.cx = int(
            settings.center_x * self.cols
        )  # get the center (axis x) of the image
        self.cy = int(
            settings.center_y * self.rows
        )  # get the center (axis y) of the image
    ...
    def compute_mask(self, downscale_factor=0.1) -> np.ndarray:
        ...
        nx_up, ny_up = self._get_vectors(
            self.settings.angle_upper
        )  # get the upper nomal
        ...
        def apply_decay(dist_field, sharp_lim, trans_lim):
            ...
            if s_trans > 0:
                mask[cond_trans] = np.minimum(
                    mask[cond_trans], 1.0 - (dist_field[cond_trans] - s_sharp) / s_trans
                )
        ...
        apply_decay(
            dist_up, self.settings.dist_upper_sharp, self.settings.dist_upper_trans
        )
        apply_decay(
            dist_low, self.settings.dist_lower_sharp, self.settings.dist_lower_trans
        )

        # 3. Upscale mask to full resolution (Linear interpolation is fast and smooth enough)
        full_mask = cv2.resize(
            mask, (self.cols, self.rows), interpolation=cv2.INTER_LINEAR
        )
        return full_mask
    ...
    def get_lines_geometry(self):
        ...
        for angle, d_sharp, d_trans in [
            (
                self.settings.angle_upper,
                self.settings.dist_upper_sharp,
                self.settings.dist_upper_trans,
            ),
            (
                self.settings.angle_lower,
                self.settings.dist_lower_sharp,
                self.settings.dist_lower_trans,
            ),
        ]:
...
def render_composite(
    img_sharp: np.ndarray, img_blur: np.ndarray, settings: MiniatureSettings
) -> np.ndarray:
...
def draw_guides_cv2(
    image: np.ndarray, settings: MiniatureSettings, active_node: str = None
) -> np.ndarray:
    overlay = image.copy()
    h, w = overlay.shape[:2]
